...
describe('creating unit', () => {
    ...
    function performAssertion(model:IModel, project:projects.IProject, done) {
        ...
        model.closeConnection(() => {
            ...
            const deltas =  (<StubClient> (<Model> model)['_client']).deltas.slice();
            writeJson('create-unit-deltas-non-prefetch.json', deltas);
            const superfluousDeltas = deltas.splice(10, 2);
            superfluousDeltas.forEach(delta => delete delta['elementId']);
            const expectedSuperfluousDeltas = [
                {
            ...
            assert.deepEqual(deltas, expectedDeltas(module, project, domainModel, folder, page));
            assert.deepEqual(superfluousDeltas, expectedSuperfluousDeltas);
            done();
        });
    }

    function expectedDeltas(module:projects.IModule, project:projects.IProject, domainModel:domainmodels.IDomainModel, folder:projects.IFolder, page:pages.Page) {
        return [
    {
