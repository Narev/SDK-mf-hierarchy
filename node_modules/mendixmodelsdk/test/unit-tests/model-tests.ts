...
import {ModelUnit, projects} from "mendixmodelsdk";
	...
	it('should be able to read all units', done => {
		createSdkClient().openWorkingCopy("1", (m) => {
			m.allUnits().forEach(unit => {
				assert.isTrue(unit instanceof ModelUnit || unit instanceof projects.Project || unit instanceof projects.FolderBase, "not a unit: " + unit.typeName);
			});
