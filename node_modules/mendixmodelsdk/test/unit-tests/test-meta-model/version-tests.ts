...
import * as version from '../../../src/sdk/internal/version-checks';
...
function createSdkClient(version:string):TestModelSdkClient {
	return TestModel.createSdkClient({
		client: new TestModelStubClient({}, version)
	});
}

type CallbackFunc = (entity:domainmodels_simple.IEntity, domainModel:domainmodels_simple.IDomainModel, model:IModel) => void;
type CallbackFuncLoaded = (entity:domainmodels_simple.Entity, domainModel:domainmodels_simple.DomainModel, model:IModel) => void;

function withEntity(mxVersion:string, done:common.IErrorCallback, doFunc:CallbackFunc) {
...
function withEntityLoaded(mxVersion:string, done:common.IErrorCallback, doFunc:CallbackFuncLoaded) {
	createSdkClient(mxVersion).openWorkingCopy("1", (model) => {
...
describe('Versioning in test meta model', () => {
	...
	it('should work for availability of structures', () => {
		const attributeAvailability = domainmodels_simple.Attribute.versionInfo.availability;
	...
	it('should work for availability of properties', () => {
		const attributesPropAvailability = domainmodels_simple.Entity.versionInfo.properties["attributes"].availability;
	...
	it('should work for modifiers of properties', () => {
		...
		const typePropertyVersionInfo = domainmodels_simple.Attribute.versionInfo.properties["type"];
	...
	function checkExperimentalCreate(done:common.IErrorCallback, forMxVersion:string, shouldWarn:boolean) {
		withEntityLoaded(forMxVersion, done, (entity, domainModel, model) => {
			...
			if (shouldWarn) {
				withWarnFunc((warning:string) => {
					assert.equal(warning, `Type 'DomainModels_simple$ExperimentalElement' is experimental in Mendix version ${forMxVersion} and should be used with care.`);
	...
	it('should detect introducedness on creation', (done) => {
		withEntityLoaded("6.0.4", done, (entity, domainModel, model) => {
			assert.doesNotThrow(() => {
				domainmodels_simple.Attribute2.createIn(entity);
			});
			done();
		});
	});
	...
	it('should detect deprecation on get', (done) => {
		withEntity("6.0.4", done, (entity, domainModel, model) => {
			withWarnFunc((warning) => {
				...
				assert.equal(warning, "Property 'attributes' of type 'DomainModels_simple$Entity' is deprecated in Mendix version 6.0.4 (deprecated since Mendix version 6.0.4) and should no longer be instantiated: Use Attribute 2 instead");
				done();
			}, () => {
	...
	it('should detect deprecation on creation', (done) => {
		withEntityLoaded("6.0.4", done, (entity, domainModel, model) => {
			...
			withWarnFunc((warning) => {
				...
				if (!isDone) { // should check isDone, because we get 2 warnings which are different. We only want to assert the first type at this moment
					assert.equal(warning, "Type 'DomainModels_simple$Attribute' is deprecated in Mendix version 6.0.4 (deprecated since Mendix version 6.0.4) and should no longer be instantiated: Use Attribute2 instead");
					isDone = true;
					done();
				}
			}, () => {
				domainmodels_simple.Attribute.createIn(entity); // should trigger console.warn
			});
		});
	});
