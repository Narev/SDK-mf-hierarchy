...
describe('version checks', () => {
	...
	it("should report lifecycle state correctly", () => {
		...
		assert.equal(fullVersioned.getCurrentState(version.determineMetaModelVersion("1.0.0-rc1")), version.LifeCycleState.AVAILABLE);
		...
		assert.equal(fullVersioned.getCurrentState(version.determineMetaModelVersion("3.0-beta")), version.LifeCycleState.REMOVED);
	...
	it("should handle modifiers lifecycles correctly", () => {
		...
		const changingModifier = new version.ModifierValueHistory({ currentValue: false, changedIn: [
			"3.0.0",
			"2.0.0",
			"1.0.0"
		]}, false);
		...
		const changingModifier2 = new version.ModifierValueHistory({ currentValue: true, changedIn: [
			"3.0.0",
			"2.0.0",
			"1.0.0",
			"0.0.2",
	...
	it("should determine meta model version correctly", () => {
		assert.equal(version.determineMetaModelVersion("1.2.3").toString(), "1.2.3"); 
		assert.equal(version.determineMetaModelVersion("1.2").toString(), "1.2.0");
		assert.equal(version.determineMetaModelVersion("1").toString(), "1.0.0");
		assert.equal(version.determineMetaModelVersion("1.2.3.beta").toString(), "1.2.3");
		assert.equal(version.determineMetaModelVersion("1.2.3-beta56").toString(), "1.2.3");
		assert.equal(version.determineMetaModelVersion("1.2.beta").toString(), "1.2.0");
		assert.equal(version.determineMetaModelVersion("1.2-beta56").toString(), "1.2.0");
	});
